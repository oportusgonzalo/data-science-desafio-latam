...
opcion = input('''Ingrese una opción para Jugar!
    ...
    > ''')
# 1. validar opcion
opcion = validate(opciones=['0', '1'], eleccion=opcion)
...
while correcto and n_pregunta < 3*p_level:
    ...
    if n_pregunta == 0:
        ...
        p_level = validate(opciones=['1', '2', '3'], eleccion=p_level)
        p_level = int(p_level)
    ...
    if continuar == 'y':
        ...
        respuesta = validate(opciones=['a', 'b', 'c', 'd'], eleccion=respuesta)
        # 8. Verificar si la respuesta es correcta o no
        correcto = verificar(alternativas, eleccion=respuesta)
        ...
        if correcto and n_pregunta < 3*p_level:
            ...
            continuar = validate(opciones=['y', 'n'], eleccion=continuar)
            os.system(op_sys)
